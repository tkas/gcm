...
GC<GCM::database::Database> openDb(const char *path) {
	...
	GCMDatabase *db = new GCMDatabase(path);
	return db;
}
		...
		virtual bool accept(const char *fileName) {
			...
			if (f) {
				...
				if (strncmp(buff, "SQLite format 3", 15) == 0) {
					...
					bool result = tableExists(stmt, "geocaches") && tableExists(stmt, "waypoints") &&
						tableExists(stmt, "attributes") && tableExists(stmt, "fieldnotes") &&
						tableExists(stmt, "geocache_long_data") && tableExists(stmt, "logs") &&
						tableExists(stmt, "points") && tableExists(stmt, "tags") && tableExists(stmt, "points_idx");
...
GC< List<GCM::database::DatabaseInfo> > getDbInfo() {
	...
	out->append(new MyDbInfo("gcm", "GCM Database"));
	return out;
}
